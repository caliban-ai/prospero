# 0002. Prospero is a control plane over caliband, not a re-implementation
...
- We avoid duplicating — and diverging from — caliban's supervision logic; spawn/kill/
  respawn/attach semantics stay defined in one place.
- Prospero's value is concentrated on the genuinely new concerns (fleet aggregation,
  history, normalization) rather than re-litigating process management.
- Prospero depends on a running `caliband` per managed repo and inherits its behavior and
  limitations; Discovery can autostart a caliband on demand to soften this.
- The boundary between the two systems must be defined precisely — see
